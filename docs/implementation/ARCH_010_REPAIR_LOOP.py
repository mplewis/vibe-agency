...
class CoreOrchestratorV2:
    ...
    def execute_phase(self, manifest: dict) -> tuple[bool, str | None]:
        ...
        if self._is_repair_mode(project_id, current_phase):
            ...
            print(
                f"🔧 REPAIR MODE: Attempt {self.repair_attempts[project_id]} with context: {repair_context}"
            )
...
def test_repair_loop():
    ...
    manifest = {"project_id": "test_repair", "current_phase": "CODING", "metadata": {}}
        ...
        def execute(self, manifest, repair_context=None):
            if repair_context:
                ...
                return SpecialistResult(
                    success=True, next_phase="TESTING", artifacts=["fixed_code.py"]
                )
            else:
                # First attempt - generate buggy code
                return SpecialistResult(
                    success=True, next_phase="TESTING", artifacts=["buggy_code.py"]
                )
    ...
    class MockTestingSpecialist:
        ...
        def execute(self, manifest, repair_context=None):
            ...
            if self.attempts == 1:
                ...
                return SpecialistResult(success=True, next_phase="AWAITING_QA_APPROVAL")
