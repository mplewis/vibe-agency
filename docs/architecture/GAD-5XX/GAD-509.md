...
**Integration Points:**
- `agency_os/00_system/runtime/llm_client.py:309` - LLM client wraps provider calls
...
```bash
# Read implementation
cat agency_os/00_system/runtime/circuit_breaker.py

# Check integration
grep -n "circuit_breaker" agency_os/00_system/runtime/llm_client.py
...
```python
from agency_os.00_system.runtime.circuit_breaker import CircuitBreaker
...
```python
from agency_os.00_system.runtime.circuit_breaker import CircuitBreaker, CircuitBreakerConfig
