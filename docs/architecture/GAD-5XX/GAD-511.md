...
│  - Adds safety layer (CB + Quota)           │
└─────────────────┬───────────────────────────┘
                  │ uses
                  ↓
┌─────────────────────────────────────────────┐
│      LLMProvider (Abstract Interface)       │
│  + invoke(prompt, model, ...) → Response   │
...
└─────────────────┬───────────────────────────┘
         ...
         ↓                 ↓          ↓         ↓
┌─────────────────┐ ┌──────────┐ ┌─────────┐ ┌──────────┐
│ AnthropicProvider│ │  Google  │ │ OpenAI  │ │   NoOp   │
│    (Claude)     │ │ (Gemini) │ │  (GPT)  │ │ (Mock)   │
└─────────────────┘ └──────────┘ └─────────┘ └──────────┘

                  Factory (Auto-Detection)
                           ↓
            Priority: Google → Anthropic → OpenAI → NoOp
```
...
**Provider System:**
```
agency_os/00_system/runtime/providers/
├── __init__.py           # Public API exports
├── base.py               # Abstract interface + NoOp
├── factory.py            # Provider creation & auto-detection
...
**Integration:**
```
agency_os/00_system/runtime/llm_client.py   # LLMClient uses providers
...
- Explicit priority order (Google → Anthropic → OpenAI → NoOp)

**Detection Logic:**
```python
Priority Order:
1. GOOGLE_API_KEY → GoogleProvider (Gemini)
2. ANTHROPIC_API_KEY → AnthropicProvider (Claude)
3. OPENAI_API_KEY → OpenAIProvider (GPT) [Future]
4. None available → NoOpProvider (Mock)
```
...
**Provider System:**
- ✅ `LLMProvider` abstract base class (base.py)
- ✅ `AnthropicProvider` for Claude (anthropic.py)
- ✅ `GoogleProvider` for Gemini (google.py)
- ✅ `NoOpProvider` fallback (base.py)
- ✅ Provider factory with auto-detection (factory.py)

**Integration:**
- ✅ `LLMClient` uses provider system (llm_client.py:235-248)
- ✅ Phoenix Config model settings (phoenix.py:177-215)
- ✅ Backward compatibility with legacy NoOpClient

**Features:**
- ✅ Uniform `invoke()` interface across providers
- ✅ Provider-specific cost calculation
- ✅ API key auto-loading from environment
...
**OpenAI Provider:**
- ⚠️ Factory recognizes `OPENAI_API_KEY` but returns NoOp
- 📋 Full implementation deferred (GAD-511 Phase 2)

**Local/Ollama Provider:**
- ⚠️ Factory recognizes local option but returns NoOp
- 📋 Full implementation deferred (GAD-511 Phase 2)
...
ls -la agency_os/00_system/runtime/providers/

# Read implementations
cat agency_os/00_system/runtime/providers/base.py
cat agency_os/00_system/runtime/providers/anthropic.py
cat agency_os/00_system/runtime/providers/google.py

# Check integration
grep -n "provider" agency_os/00_system/runtime/llm_client.py | head -20
...
```python
from agency_os.00_system.runtime.providers import create_provider, AnthropicProvider
from agency_os.00_system.runtime.llm_client import LLMClient
